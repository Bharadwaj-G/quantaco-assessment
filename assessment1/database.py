"""Plain psycopg2 connections to Cloud SQL for Postgres.
...
Two ways in, same driver, no Cloud SQL client library needed:
  ...
  - Locally: run the Cloud SQL Auth Proxy (a standalone binary) to open a
    local TCP port tunneled to the instance, then set DB_HOST/DB_PORT to
    point at it (typically 127.0.0.1:5432). Same psycopg2 code path either
    way — only the connection target changes.

Required env vars: DB_USER, DB_PASS, DB_NAME, plus either
INSTANCE_UNIX_SOCKET (Cloud Run) or DB_HOST + DB_PORT (local, via proxy).
"""
